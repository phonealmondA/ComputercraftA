...
local CONFIG = {
    ...
    DEST_SIDE = "left",
    WIRELESS_SIDE = "top",
    OUTPUT_CHEST = "minecraft:chest_42",
    REFRESH_INTERVAL = 1,
	hackerId = 24
}
...
if not peripheral.isPresent(CONFIG.WIRELESS_SIDE) then
    error("No modem found on " .. CONFIG.WIRELESS_SIDE .. " side!")
end

rednet.open(CONFIG.SOURCE_SIDE)
rednet.open(CONFIG.DEST_SIDE)
rednet.open(CONFIG.WIRELESS_SIDE)
...
parallel.waitForAll(
    ...
    -- Single request handling thread for both wired and wireless
    function()
        while true do
            ...
            if message and type(message) == "table" then
                if message.type == "list" then
                    ...
                    --rednet.send(CONFIG.hackerId, { type = "inventory", items = getInventoryList(), computer = os.getComputerID()}, CONFIG.REQUEST_PROTOCOL)
